...
class ShapeOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy = True):
        ...
        for i in range(len(node.reduction_indices)):
            ...
            return output_val * input_shape[idx]
    def gradient(self, node, output_grad):
        return [0]
    def infer_shape(self, node, input_shapes):
...
class ReshapeOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy = True):
        assert len(input_vals) == 1
        return input_vals[0].reshape(node.to_shape)
    def gradient(self, node, output_grad):
...
class ArgmaxOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy = True):
        assert len(input_vals) == 1
        return np.argmax(input_vals[0], axis = node.reduction_indices)
    def gradient(self, node, output_grad):
        return [0]
    def infer_shape(self, node, input_shapes):
        node.keepdims = False
...
class PowerOp(Op):
    ...
    def gradient(self, node, output_grad):
        assert False
        return [node.inputs[1] * node / node.inputs[0], log_op(node.inputs[0]) * node]
    def infer_shape(self, node, input_shapes):
        assert len(input_shapes) == 2
    ...
    def compute(self, node, input_vals, output_val, use_numpy = True):
        if use_cpp:
            ...
            if node.matmul_attr_trans_a:
                ...
                k = A.shape[1]
            lib.matmul(A_data, B_data, C_data, node.matmul_attr_trans_a, node.matmul_attr_trans_b, m, k, n)
            return C
        else:
...
class ReduceSumToOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy = True):
        ...
        for i in range(len(output_shape)):
            while(i < len(tmp.shape) and tmp.shape[i] != output_shape[i]):
...
class Conv2dGradientWOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True):
        ...
        if not use_cpp:
            ...
            return DW_col.reshape((fil_h, fil_w, in_ch, ou_ch))
        else:
...
class DropoutOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True):
        ...
        return input_vals[0] * node.keep_status
...
class DropoutGradientOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True):
        assert len(input_vals) == 1
        return input_vals[0] * node.origin.keep_status
        ...
        for node, value in feed_dict.items():
            if use_numpy:
                ...
                if isinstance(value, np.ndarray):
                    node_to_val_map[node] = np.ndarray.array(value, ctx=self.ctx)
                elif isinstance(value, np.ndarray.NDArray):
                    node_to_val_map[node] = value
                else:
