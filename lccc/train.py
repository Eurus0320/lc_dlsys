...
class GradientDescentOptimizer:
    ...
    def minimize(self, cost_function):
        assert isinstance(cost_function, ops.Node)
        topo_order = ops.find_topo_sort([cost_function])
        para = []
        for node in topo_order:
            if isinstance(node.op, ops.VariableOp):
                para.append(node)
        grad = gradients(node, para)
        assign_nodes = []
        for i in range(len(para)):
            assign_nodes.append(assign(para[i], para[i] - self.learning_rate * grad[i]))
        optimizer = ops.init_op(assign_nodes)
        return optimizer
